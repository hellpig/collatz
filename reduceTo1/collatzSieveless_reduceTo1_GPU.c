...
If you want to count it as 2 steps, there are 4 lines of code
  to uncomment or switch to: 3 in kernel1_2 and 1 in kernel2
...
On Linux, compile via something like...
  clang -O3 collatzSieveless_reduceTo1_GPU.c -lOpenCL
...
/*
  For kernel1 and kernel1_2 that make the sieves...
...
*/
const int TASK_SIZE_KERNEL2 = 47;
...
/*
  ...
  Not to be updated until the next TASK_ID_KERNEL2
*/
int STEPS_MAX = 762;
...
// set kernel files
static const char *kernel1   = "kernel1_reduceTo1.cl";   // for k1
static const char *kernel1_2 = "kernel1_2_reduceTo1.cl"; // for k2
static const char *kernel2   = "kernel2_reduceTo1.cl";   // for actually using sieves
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  gettimeofday(&tv1, NULL);
	...
	cl_mem mem_obj_arrayDelay;    // for k2 to store delays
	...
	size_t arrayDelayCount = (size_t)1 << k2;               // for k2
	size_t arrayIncreasesCount = (size_t)1 << TASK_SIZE;
	size_t indicesCount = (size_t)1 << (TASK_SIZE - 0);     // not all of this will be used
	...
	if ( arrayLarge == NULL || arrayIncreases == NULL || indices == NULL ) {
		return -1;
	}
...
next_platform:
	...
	for (; (cl_uint)device_index < num_devices; ++device_index) {
		...
		mem_obj_arrayDelay = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_ushort) * arrayDelayCount, NULL, &ret);
		...
		mem_obj_arrayIncreases = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_ulong) * arrayIncreasesCount, NULL, &ret);
		...
		ret = clSetKernelArg(kernel1, 0, sizeof(cl_mem), (void *)& mem_obj_arrayLarge);

		if (ret != CL_SUCCESS) {
			return -1;
		}

		ret = clSetKernelArg(kernel1, 1, sizeof(cl_mem), (void *)&mem_obj_arrayIncreases);
		...
		ret = clSetKernelArg(kernel1_2, 1, sizeof(cl_mem), (void *)& mem_obj_arrayDelay);
		...
		sprintf(options2, "%s -D SIEVE_LOGSIZE=%i -D SIEVE_LOGSIZE2=%i -D TASK_SIZE=%i -D TASK_ID=%" PRIu64 " -D TASK_SIZE_KERNEL2=%i -D TASK_ID_KERNEL2=%" PRIu64 " -D STEPS_MAX=%i",
			g_ocl_ver1 ? "" : "-cl-std=CL2.0",
			k,
			k2,
			TASK_SIZE,
		...
		ret = clSetKernelArg(kernel2, 4, sizeof(cl_mem), (void *)& mem_obj_arrayDelay);
  ...
  __uint128_t n0start = (__uint128_t)TASK_ID * arrayIncreasesCount;  // for analyzing the results of kernel1
  ...
  if (TASK_ID > 0) {
    for (size_t i = 0; i < deltaN; i++) {
      ...
      __uint128_t n = n0start - deltaN + i;
