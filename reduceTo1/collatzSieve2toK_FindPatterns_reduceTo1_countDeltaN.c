...
Compile and run via something like...
  clang -O3 collatzSieve2toK_FindPatterns_reduceTo1_countDeltaN.c
...
int main(void) {
  ...
  /* the following doesn't checkpoint yet */
  ...
  gettimeofday(&tv1, NULL);
	...
	const size_t patternsPerArray = arrayIncreasesCount;
  ...
  for (uint64_t task_id = task_id_group * tasksPerSave; task_id < (task_id_group + 1) * tasksPerSave; task_id++) {
    ...
    for (size_t index = 0; index < patternsPerArray; index++) {
  ...
  printf(" total deltaN count = %llu\n", (unsigned long long)totalDeltaNcount);
  for (size_t i=deltaN; i>0; i--) {
    if (deltaNcounts[i] > 0)
      printf(" %llu: %llu\n", (unsigned long long)i, (unsigned long long)deltaNcounts[i]);
  }
