...
On Linux, compile via something like...
  clang -O3 collatzPartiallySieveless_repeatedKsteps_GPU.c -lOpenCL
...
next_platform:
	...
	for (; (cl_uint)device_index < num_devices; ++device_index) {
		...
		sprintf(options2, "%s -D SIEVE_LOGSIZE=%i -D SIEVE_LOGSIZE2=%i -D TASK_SIZE=%i -D TASK_ID=%" PRIu64 " -D TASK_SIZE_KERNEL2=%i -D TASK_ID_KERNEL2=%" PRIu64,
			g_ocl_ver1 ? "" : "-cl-std=CL2.0",
			k,
			k2,
			TASK_SIZE,
  ...
  */
  ...
  fseek(fp, ((((uint64_t)1 << k1) - 1) & bStart) >> 7, SEEK_SET);
	...
	*/
	...
	/* wait for kernel1 to finish (not exactly necessary yet) */
  ...
  for (int j = 0; j < (global_work_size2 % 32); j++) {
	...
	clFinish(command_queue);
        ...
        /*
	   Wait for kernel2 to finish without Nvidia causing CPU busy waiting.
	   Basically, just use usleep() in a loop that checks if the kernel is done.
	   This does not work on my Nvidia GPU when kernel2 contains a printf().
