/*
  ...
  In 2021, I, Brad Knockel, am making the following improvements to this file...
    - I no longer include cuda.h because nvcc would do that for you;
      cuda.h was causing issues for CPU-only code
    - add128(uint128_t x, uint64_t y) no longer is missing commas in CUDA asm()
    - operator>>=() and operator<<=() now correctly handle a shift of 0
    - div128to64() now gives the correct answer when divisor is 2^63 or larger
    - operator+=() now has a version that has the 2nd addend be uint64_t
      to match the add128() that does the same, and neither no longer
      copy to a variable called temp
    - operator-=() incorrectly handled uint128_t subtrahend and unnecessarily
      copied the subtrahend to a variable called temp, so I fixed it
    - operator-=() now has a version that has the subtrahend be uint64_t
    - sub128() now has a version that has the subtrahend be uint64_t
    - operator*=() now exists, though untested on ARM

  I found this file here before I modified it...
    https://github.com/curtisseizert/CUDA-uint128
  When I notice that the above link fixes something,
  I remove it from my list above.

  For my Collatz code, I then...
    - commented out parts of my operator>>=() since I never
      need to shift by an amount other than 0 < b < 64, which
      requires that 0 < (TASK_SIZE_KERNEL2 - k) < 64
      (actually, b==0 works if hi==0)
    - commented out parts of my operator<<=() since I never
      need to shift by an amount other than 0 < b < 64, which
      requires that 0 < (TASK_SIZE_KERNEL2 - k) < 64
      and than k not be larger than 63.
      k not being too large is the important restriction.
    - commented out 4 lines just after "public:" because I would never need
      a default initialization, and I don't want to have to use this class
      differently on CPU vs GPU
...
template <typename T>
  ...
  __host__ __device__
#endif
    friend uint128_t operator-(uint128_t a, const T & b){return sub128(a, b);}
...
*/
    ...
    static inline uint128_t sub128(uint128_t x, uint128_t y) // x - y
  {
    ...
    __host__ __device__
  #endif
    static inline uint128_t sub128(uint128_t x, uint64_t y) // x - y
  {
    uint128_t res;
  ...
  inline uint128_t add128(uint128_t x, uint128_t y)
{
  ...
  __host__ __device__
#endif
  inline uint128_t add128(uint128_t x, uint64_t y)
{
  return uint128_t::add128(x, y);
}
  ...
  inline uint128_t sub128(uint128_t x, uint128_t y) // x - y
{
  ...
  __host__ __device__
#endif
  inline uint128_t sub128(uint128_t x, uint64_t y) // x - y
{
  return x - y;
}
