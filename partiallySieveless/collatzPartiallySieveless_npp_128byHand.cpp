...
You'll need a 64-bit computer with a not-ancient version of g++
  in order for __uint128_t to work. This gives a 128-bit integer!
Compile using something like...
  g++ -O3 collatzPartiallySieveless_npp_128byHand.cpp

I use the following to speed up my code...
  __builtin_ctz(n)
  __builtin_ctzll(n)
On my computer, an "unsigned long long" is only 64 bits (not 128 bits).
Also, I do things like the following because ctz(0) is undefined...
            alpha = (int)n.lo ? __builtin_ctz(n.lo) : lenC3 - 1;
...
/*
  So that the following can be uint128_t instead of __uint128_t...
    b, bStart, bb, bm
    a, aStartByHand, aEndByHand
    n, nStart, maxNs[]
*/
...
int main(int argc, char *argv[]) {
  ...
  //const int lenC3 = 41;  // for 64-bit numbers, which uint128_t requires
  const int lenC3 = 33;    // to not have to worry about using min() after __builtin_ctz()
...
*/
  ...
  uint64_t* c3 = (uint64_t*)malloc(lenC3*sizeof(uint64_t));
  c3[0] = 1;
  ...
  uint128_t* maxNs = (uint128_t*)malloc(lenC3*sizeof(uint128_t));
  for (j=0; j<lenC3; j++) { maxNs[j] = div128to128((~(uint128_t)0), c3[j]); }
  ...
  const __uint128_t maxA = (UINTmax - temp + 1) / temp;     // max a
  if (aEnd - 1 > maxA) {
  ...
  uint128_t bStart = ( (uint128_t)1 << TASK_SIZE )*task_id;
  ...
  /*
    ...
    Note that ((((uint64_t)1 << k1) - 1) & bStart.lo) equals bStart % ((uint64_t)1 << k1)
  */
  //rewind(fp);
  fseek(fp, ((((uint64_t)1 << k1) - 1) & bStart.lo) >> 7, SEEK_SET);
  ...
  // aStartByHand and aEndByHand are the uint128_t versions of aStart and aEnd
  uint128_t aStartByHand;
  aStartByHand.lo = (uint64_t) aStart;
  aStartByHand.hi = (uint64_t) (aStart >> 64);
  uint128_t aEndByHand;
  aEndByHand.lo = (uint64_t) aEnd;
  aEndByHand.hi = (uint64_t) (aEnd >> 64);
  ...
  for (uint64_t pattern = 0; pattern < ((uint64_t)1 << (TASK_SIZE - 8)); pattern++) {
    ...
    for (int bit = 0; bit < 16; bit ++) {      // loop over 16 bits in pattern
      ...
      uint64_t r = 0;
      r += b.lo          & 0xfffffffffffffff;
      r += (b >> 60).lo  & 0xfffffffffffffff;
      r += b.hi >> 56;       // b >> 120
      bMod = r%9;

      for (a = aStartByHand; a < aEndByHand; ++a) {    // loop over a before next b (interlacing for speed)
          ...
          while (1) {             // go until overflow or n < nStart
            ...
            while (n.lo & 1) {     // make sure n is odd to prevent a bit shift by 0
                ++n;
                alpha = (int)n.lo ? __builtin_ctz(n.lo) : lenC3 - 1;
                n >>= alpha;                   // requires 0 < alpha < 64
                if ( n > maxNs[alpha] ) {
                  printf("Overflow! nStart = ");
                  print128( ((__uint128_t)nStart.hi << 64) | nStart.lo );
                  break;
                }
                n *= c3[alpha];     // requires alpha < 41
                --n;
            }

            if (n.lo == 0) n = (uint128_t)n.hi;   // shift by 64
            else n >>= __builtin_ctzll(n.lo);     // 0 < shift < 64
            if (n < nStart) break;
